...
export const createWorkoutPlanService = async (input) => {
...
export const getWorkoutPlanByDateService = async (dateStr) => {
  const date = new Date(dateStr).toISOString().split("T")[0];
  return await workoutPlan.findOne({ date });
};
...
export const getWorkoutPlansService = async (dateStr) => {
  const date = new Date(dateStr).toISOString().split("T")[0];
  return await workoutPlan.find({ date });
};

export const getWorkoutPlanByIdService = async (id) => {
  return await workoutPlan.findOne({ _id: id });
};
